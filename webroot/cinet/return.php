...
try 
{
    ...
    $designation=$infopayeur[1];
     $dat = date("YmdHis");
    echo $payment_method,$infopayeur[0],$infopayeur[1];
	...
	if($designation=='Paiement abonnement' && $resultat==00 && $trans_status=='ACCEPTED' && $resultat==00 && $trans_status=='ACCEPTED')
	{
		...
		{                    //insertion dans la table paiementab
			...
			if($query->execute( array( 'id'=>'', 'id_transaction'=>$id_transaction, 'montant'=>$montant, 'devise'=>'F CFA', 'site_id'=>$site_id, 'langue'=>$langue,
								...
								'designation'=>$designation,'dat'=>$dat )))
			{   
								//insertion dans la table abonnement                                    
								$ptcom = 10/100;
								$montant = $montant/1.055;
								...
								$query3->execute();
								//SELECT dans la table retrait pour faire la somme des montants enregistré
								$query10= $bdd->query("SELECT SUM(montant) FROM retrait");$solderetrait=$query10->fetch();
								echo  $solderetrait[0];                                   
								$solderetrait = $solderetrait[0];
								...
								$query9 = $bdd->prepare('INSERT INTO comptesl SET soldeab = :soldeab, soldecomcash = :soldecomcash, soldetva = :soldetva, capitalsl = :capitalsl, dat = NOW()');
							   ...
							   $query11 = $bdd->prepare('INSERT INTO histocompte SET compte = :compte, montant = :montant, datchgt = NOW()');
							   $query11->bindValue(':compte', $cptparrain[0], PDO::PARAM_INT);
							   $query11->bindValue(':montant', $cptparrain[2]);
							   $query11->execute();
							   echo 'solde du compte cash du parrain ds histocompte'.$cptparrain[2].'<br>';
							   ...
							   $commission = $ptcom*$montant;
								...
								$query7->execute();
								 ...
								 $_SESSION['Membre']->id = $idmbre;
								 $_SESSION['Membre']->etat = 'abonner';
								 echo'session du membre'. $_SESSION['Membre']->id;
								 //die();
								 header('location:https://www.smarts-life.com/sl/dashboard');
								 //header('location:../sl/dashboard');
			}
			else{echo 'deja enregistrer';}
		}
	...
	if($cpp==0)
	{
		...
		if($query->execute(array('', $idmbre, $idcmd, 'CinetPay', $montant, $id_transaction)))
		{
			...
			header('location: https://www.smarts-life.com/bitchange/?page=message');
			//echo 'id'.$_SESSION['client']['id'];
			//die();
			//header('location:../bitchange/?page=message');
		}
	}
...
{ 
	...
	header('location:https://smarts-life.com/sl/dashboard');
}
